...
const OrderCountValue = () => {
  ...
  const getOrders = async (params) => {
    try {
      ...
      const response = await reportService.getMostSellingProducts(token, query.toString())
  ...
  const { title, totalQuantity } = orders
  if (!totalQuantity) return
  const data = {
    labels: title,
    datasets: [{
      label: "Total Quantity",
      data: totalQuantity.map(x => parseInt(x)),
    ...
    <div className="card mb-4">
      <div className="card-header bg-white">
        <div className="row align-items-center">
          <div className="col-md-6 fw-bold">Highest Selling Product by <span className="text-capitalize">{by[ordersBy]}</span></div>
      ...
      <div className="card-body">
        {orders && <Bar options={options} data={data} height={75} />}
      </div>
    </div>
  )
}
